...
namespace NBTExplorer.Windows
{
    ...
    public partial class MainForm : Form
    {
        ...
        {
            if (!ConfirmAction("是否打开此文件？此操作将覆盖当前界面！"))
                return;

            using (OpenFileDialog ofd = new OpenFileDialog() {
                RestoreDirectory = true,
                Multiselect = true,
                Filter = "所有文件|*|NBT 文件 (*.dat, *.schematic)|*.dat;*.nbt;*.schematic|Region 文件 (*.mca, *.mcr)|*.mca;*.mcr",
                FilterIndex = 0,
            }) {
        ...
        {
            if (!ConfirmAction("是否打开此文件夹？此操作将覆盖当前界面！"))
                return;
        ...
        private void OpenPaths (string[] paths)
        {
            ...
            if (failCount > 0) {
                MessageBox.Show("一个或者多个问题阻止打开它。请稍后重试。(" + failCount + ")");
            }
        }
        ...
        {
            if (!ConfirmAction("打开Minecraft目录？此操作将覆盖当前界面！"))
                return;
        ...
        private bool ConfirmExit ()
        {
            if (_controller.CheckModifications()) {
                if (MessageBox.Show("您的文件尚未保存。是否继续退出？", "未保存的改变", MessageBoxButtons.OKCancel) != DialogResult.OK)
                    return false;
            }
        ...
        {
            if (_controller.CheckModifications()) {
                if (MessageBox.Show("你的项目有未保存的地方。  " + actionMessage, "Unsaved Changes", MessageBoxButtons.OKCancel) != DialogResult.OK)
                    return false;
            }
        ...
        {
            if (_controller.CheckModifications()) {
                if (MessageBox.Show("你的项目有未保存的地方。  " + e.Message, "Unsaved Changes", MessageBoxButtons.OKCancel) != DialogResult.OK)
                    e.Cancel = true;
            }
        }
		...
		private void h帮助ToolStripMenuItem_Click(object sender, EventArgs e)
		{
            new UseHelp().ShowDialog();
        }
